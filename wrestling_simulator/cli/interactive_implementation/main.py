...
class Wrestlers_Interactive:
        ...
        while not valid:
            ...
            match icmd:
                ...
                case "view":
                    try:
                        val=int(req.split(" ")[1])
                        print(plyrs[val-1].display_stats_table())
                    except (IndexError,ValueError):
                        print("please refer to help command")
                ...
                case "compare":
                    ...
                    if len(values)==2:
                        values=[int(i) for i in values]
                        data=Wrestler.compare_wrestlers(plyrs[values[0]-1],plyrs[values[1]-1])
                        print(data)
                    else:
    ...
    def game_loop(self):
        ...
        Tournament.match(None,self.current,self.current_bot,self.player_action,False)
    ...
    def player_action(self):  
        ...
        while True:
            ...
            match mov:
                ...
                case "stats":
                    print(self.current.display_stats_table())
                ...
                case "compare":
                    ...
                    if len(values)==2:
                        values=[int(i) for i in values]
                        data=Wrestler.compare_wrestlers(self.players[values[0]],self.players[values[1]])
                        print(data)
                    else:
                        print("invalid command usage please refer to help command")

                case "tag":
                    bench_players=[p for i,p in enumerate(self.players) if p.name != self.current.name]
